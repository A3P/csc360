...
int main(int argc, char *argv[]) {
    ...
    //child goes through because it's pid = 0 parent has the pid of the child
    if ((pid = fork()) == 0) {
        printf("child: about to start...\n");
      
        // Create if does not exist, open for read and write, User read, User write
        fd = open("output.txt", O_CREAT|O_RDWR, S_IRUSR|S_IWUSR);
        if (fd == -1) {
            fprintf(stderr, "cannot open output.txt for writing\n");
            exit(1);
        }
        // forces file descriptor in stream 1 aka stdout to be connected to the file we opened
        dup2(fd, 1);
        dup2(fd, 2); 
    ...
    // Specifically waits for a specific pid, stores the message in status
